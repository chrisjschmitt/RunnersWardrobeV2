...
export function StartRun({ apiKey, hasApiKey, temperatureUnit, thermalPreference = 'average', onNeedApiKey, testMode = false, testWeather, activity = 'running' }: StartRunProps) {
  ...
  const [isUsingManualWeather, setIsUsingManualWeather] = useState(false); // Track if using manual weather
  ...
  // Helper to convert TestWeatherData to WeatherData
  const convertTestWeatherToWeatherData = (tw: TestWeatherData, location: string): WeatherData => {
    ...
    try {
      ...
      if (testMode && testWeather) {
        ...
        try {
          ...
          // If fetch fails, check for manual weather
          const settings = await getSettings();
          if (settings?.manualWeather) {
            // Use manual weather
            weatherData = convertTestWeatherToWeatherData(settings.manualWeather, '📱 Manual Entry');
            // Don't set error - we successfully used manual weather
          } else {
            // No manual weather available, throw the original error
            throw fetchErr;
          }
        }
      }

      // Check if we're using manual weather
      setIsUsingManualWeather(weatherData.location === '📱 Manual Entry');
      ...
      // Check if manual weather is available as a fallback
      try {
        const settings = await getSettings();
        if (settings?.manualWeather) {
          // Use manual weather
          const weatherData = convertTestWeatherToWeatherData(settings.manualWeather, '📱 Manual Entry');
          
          setIsUsingManualWeather(true);
          setWeather(weatherData);
          setLastUpdate(new Date());
          ...
          // No manual weather available, show error with suggestion
          setError('Failed to load weather. Set manual weather in Settings to use the app offline.');
        }
      } catch (settingsErr) {
        // Settings read failed, show original error
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Manual weather indicator */}
      {isUsingManualWeather && weather && (
        <div className="glass-card p-4 mb-4 border border-blue-500/50 bg-[rgba(59,130,246,0.2)] animate-fade-in">
          <div className="flex items-start gap-3">
            <svg className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <div className="flex-1">
              <p className="text-blue-300 font-medium text-sm">Using Manual Weather</p>
              <p className="text-xs text-[var(--color-text-muted)] mt-1">
                Internet connection unavailable. Using weather data from Settings. Update it in Settings if conditions change.
              </p>
            </div>
          </div>
        </div>
